...
Page({
  ...
  getData: function () {
    ...
    return new Promise((resolve, reject) => {
      let res = wx.getStorageSync('groupList');
      if (res.carts) {
        let carts = res.carts;
        for (let n in carts) {
          arr.push({
        ...
        that.setData({
          isAlone: isAlone,
          carts: arr,
          allFreight: arr[0].datas.list[0].freight,
          totalGoodsMoney: arr[0].datas.list[0].groupPrice,
  ...
  toggleAddress: function () {
    wx.navigateTo({
      url: "/pages/mine-address/mine-address?order=" + 'true'+'&orderType=group',
    })
  },
    ...
    if (isSelectGoodsId) {
      isSelectGoodsId.map((val, index) => {
        if (val.isSelect != 1) {
          ...
          let groupInfo = wx.getStorageSync('groupList').groupInfo;
          obj = {
            orderSrc: orderSrc,
            userId: userId,
            ...
            is_group: 1,
            groupNo: groupInfo.groupNo || 0,
            buyNum: groupInfo.buyNum,
            goodsId: groupInfo.goodsId || 0,
            grouponid: groupInfo.grouponid || 0,
            grouponNum: groupInfo.groupNum || 0,
            specstring: groupInfo.specstring || 0,
          };
