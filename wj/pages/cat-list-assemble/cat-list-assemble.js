// pages/cat-list-assemble/cat-list-assemble.js
...
Page({
  ...
  toggleAd: function (typeArgu) {
    ...
    app.util.request(app.api.grouGoods, 'GET', { type: typeArgu }).then((res) => {
      console.log(res)
      if (res.status && res.status == 1 && res.goods.length != 0) {
  ...
  goCatDetail:function(e){
    ...
    wx.navigateTo({
      url: "/pages/cat-list-assemble/cat-detail-assemble?goodsId="+e.currentTarget.dataset.id
    })
  }
})